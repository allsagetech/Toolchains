...
function Test-ProductionReadinessPolicies {
	...
	Assert-True ($workflowText -match "@\('--timeout', '60s'") 'Cosign verification does not set a bounded internal timeout.'
	Assert-True ($workflowText -match 'Verify signature and attestations fail closed[\s\S]+timeout-minutes:\s+10') 'Cosign verification step has no GitHub-enforced deadline.'
	Assert-True ($workflowText -match 'RedirectStandardOutput = \$true' -and $workflowText -match 'RedirectStandardError = \$true') 'Cosign verification does not drain child-process output asynchronously.'
	Assert-True ($workflowText -match '\$process\.WaitForExit\(\$TimeoutSeconds \* 1000\)') 'Cosign verification has no external hard timeout.'
	Assert-True ($workflowText -match '\$process\.Kill\(\$true\)' -and $workflowText -match '\$process\.WaitForExit\(10000\)') 'Cosign verification does not terminate and reap timed-out process trees.'
