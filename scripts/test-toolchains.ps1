...
function Test-ProductionReadinessPolicies {
	...
	Assert-True ($monitorWorkflowText -match "TOOLCHAIN_COSIGN_VERIFY:\s*'1'" -and $monitorWorkflowText -match 'Install-VerifiedCosign\.ps1') 'Health monitoring does not fail closed on signed-catalog verification.'
	Assert-True ($monitorWorkflowText -match '\(\[bool\]\$report\.healthy\)\.ToString\(\)\.ToLowerInvariant\(\)') 'Health monitoring does not emit a canonical boolean workflow output.'
	...
	Assert-True ($workflowText -match 'Resolve-TlcManualPackageSelection -Selector \$selector') 'Manual publication does not resolve canonical multi-platform families.'
	Assert-True ($workflowText -match 'Atomic family ''\$\(\$selection\.CanonicalName\)'' cannot publish because one or more platform members are quarantined') 'Atomic family publication does not fail closed when a platform member is quarantined.'
	Assert-True ($workflowText -match "platform-index-plan:[\s\S]+needs: \[init, validate, release\][\s\S]+needs\.release\.result == 'success' \|\| needs\.release\.result == 'skipped'") 'Platform index publication is not gated on the complete family release matrix.'
	...
	Assert-True ($workflowText -match "cosign SBOM artifact upload[\s\S]+'upload', 'blob'[\s\S]+'application/spdx\+json'[\s\S]+cosign SBOM artifact sign") 'Full SPDX documents are not uploaded and signed as durable OCI artifacts.'
	Assert-True ($workflowText -match 'cosign SBOM reference attestation[\s\S]+''--type'', \$sbomReferenceType[\s\S]+''--predicate'', \$sbomReferencePath') 'Published images do not carry a compact digest-bound SBOM reference attestation.'
	Assert-True ($workflowText -notmatch "'attest'[^\r\n]+\.spdx\.json") 'Publication still sends the full SPDX document to Rekor instead of its compact reference predicate.'
	Assert-True ($cosignEvidenceText -match 'FromBase64String' -and $cosignEvidenceText -match 'predicate\.sbom\.artifact\.digest\.sha256') 'SBOM reference verification does not validate the signed in-toto predicate structure.'
	Assert-True ($cosignEvidenceText -match 'Published SBOM artifact signature verification') 'The external full SPDX artifact is not independently signature-verified.'
	...
	Assert-True ($cosignEvidenceText -match '''--output-file'', \$outputFile' -and $cosignEvidenceText -match '\[IO\.File\]::Delete\(\$outputFile\)') 'Cosign verification can flood runner logs with large attestation payloads or retain temporary output.'
	...
	Assert-True ($workflowText -match '\$buildxCreateAvailable = [\s\S]+Usage:\\s\+docker buildx imagetools create[\s\S]+\$createArguments \+= ''--prefer-index=false''[\s\S]+& docker @createArguments') 'Platform alias publication does not conditionally use verified Buildx capabilities.'
	...
	Assert-True ($workflowText -match 'docker manifest compatibility mode[\s\S]+docker manifest create \$leafTag \$digestRef[\s\S]+docker manifest push \$leafTag') 'Platform alias publication has no standard Docker manifest fallback when Buildx is unavailable.'
	Assert-True ($workflowText -match 'docker manifest inspect --verbose \$immutableTag[\s\S]+Descriptor\.platform\.os[\s\S]+Descriptor\.platform\.architecture[\s\S]+\$platformManifests\.Count -ne 1') 'Platform alias repair does not select exactly one matching digest from an existing multi-platform index.'
	...
	$ghText = Get-Content -LiteralPath .\src\pkgs\gh.ps1 -Raw
	Assert-True ($ghText -match 'Invoke-TlcVerifiedGoCommandBuild') 'GitHub CLI still packages the vulnerable upstream binary archive.'
	Assert-True ($ghText -match "GoToolchain = 'go1\.26\.6'") 'GitHub CLI does not pin the fixed Go toolchain.'
	foreach ($patchedModule in @(
		@{ Property = 'PatchedCryptoVersion'; Version = 'v0.54.0' },
		@{ Property = 'PatchedNetVersion'; Version = 'v0.56.0' },
		@{ Property = 'PatchedTextVersion'; Version = 'v0.40.0' },
		@{ Property = 'PatchedGrpcVersion'; Version = 'v1.82.1' },
		@{ Property = 'PatchedRekorVersion'; Version = 'v1.5.3' }
	)) {
		Assert-True ($ghText -match "$($patchedModule.Property) = '$([regex]::Escape($patchedModule.Version))'") "GitHub CLI does not pin $($patchedModule.Property) to $($patchedModule.Version)."
	}
