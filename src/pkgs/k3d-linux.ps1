...
function global:Install-TlcPackage {
	...
	try {
		...
		Invoke-TlcNativeCommand -FilePath $go `
			-ArgumentList @('build', '-trimpath', '-ldflags', $ldflags, '-o', (Get-TlcPkgPath 'k3d'), '.') `
	...
	if (-not (Test-Path -LiteralPath $executable -PathType Leaf)) { throw 'k3d source build did not produce k3d' }
	& chmod '+x' $executable
